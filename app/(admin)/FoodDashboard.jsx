...
import { useFetchFoodApi } from "../../ReactQuery/hooks/useFetchFoodApi";
...
export default function FoodDashboard() {
 ...
 const { data: apiData, isLoading, error: apiError, refetch } = useFetchFoodApi(
  `Food/GetFoodDetailsByDate?st_dt=${currentDate}`,
  user?.token,
  {
    refetchInterval: 60000, // Poll every 60 seconds
    refetchIntervalInBackground: true, // Optional: keep polling when app is backgrounded
  }
);
...
const {
...
} = useFetchFoodApi(`Resource/GetResourceCount`, user?.token, {
  ...
  console.log("All Food Requests:", FoodApiData?.assests_requests);
  console.log("Opted for Food:", OptforFood);
  console.log("Not Opted for Food:", NotOptForFood);
...
// 
// const summaryText = `
//     Out of <b>${Emp.teamMembers}</b> people, 
//     <b>${OptforFood.length}</b> prefer office food, 
//     <b>${NotOptForFood.length}</b> said no office food, and 
//     <b>${NotDecided}</b> made no choice.
//   `;